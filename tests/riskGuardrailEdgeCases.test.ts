...
describe('theme classification', () => {
  ...
  it('classifies crypto symbols', () => {
    const result = runRiskGuardrailEngine({
      signals: [makeSignal({ signal_id: 'c1', symbol: 'BTC-USDT', market: 'CRYPTO', asset_class: 'CRYPTO' })],
      riskState: makeRiskState()
    });
...
describe('recommendation state machine', () => {
  ...
  it('STAY_OUT when budget >= 95%', () => {
    ...
    const result = runRiskGuardrailEngine({
      signals: [makeSignal({ position_advice: { position_pct: 50 }, entry_zone: { low: 100, high: 102 }, stop_loss: { price: 90 } })],
  ...
  it('only counts NEW and TRIGGERED signals for risk', () => {
    const result = runRiskGuardrailEngine({
      signals: [
        ...
        makeSignal({ signal_id: 'expired', status: 'EXPIRED', position_advice: { position_pct: 10 } })
      ],
      riskState: makeRiskState()
    });
    ...
    expect(result.portfolio_risk_budget.used_risk_pct).toBe(withExpired.portfolio_risk_budget.used_risk_pct);
  });
});
