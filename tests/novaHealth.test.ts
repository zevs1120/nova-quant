...
describe('nova local health', () => {
  ...
  it('inspects local Ollama reachability and model availability', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response(
            JSON.stringify({
              data: [{ id: 'qwen3:8b' }, { id: 'qwen3:4b' }, { id: 'qwen3-embedding:0.6b' }],
            }),
            {
              status: 200,
              headers: {
    ...
    expect(
      report.expected_models.some(
        (row) => row.alias === MARVIX_MODEL_ALIASES.core && row.available,
      ),
    ).toBe(true);
    expect(
      report.expected_models.some(
        (row) => row.alias === MARVIX_MODEL_ALIASES.challenger && !row.available,
      ),
    ).toBe(true);
