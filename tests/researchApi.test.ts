...
describe('research api contracts', () => {
  it('serves factor catalog and factor definitions', async () => {
    ...
    const doctrineRes = await requestLocalHttp(app, { path: '/api/research/doctrine' });
    expect(doctrineRes.status).toBe(200);
    expect(doctrineRes.body.doctrine?.market_scope?.priority?.[0]).toBe('COMMODITY_FUTURES');

    const catalogRes = await requestLocalHttp(app, { path: '/api/research/factors' });
    expect(catalogRes.status).toBe(200);
    expect(Array.isArray(catalogRes.body.records)).toBe(true);
    expect(
      catalogRes.body.records.some((row: { factor_id: string }) => row.factor_id === 'momentum'),
    ).toBe(true);

    const factorRes = await requestLocalHttp(app, { path: '/api/research/factors/momentum' });
    expect(factorRes.status).toBe(200);
    expect(factorRes.body.factor?.factor_id).toBe('momentum');

    const measuredRes = await requestLocalHttp(app, {
      path: '/api/research/factors/momentum/measured',
      query: { market: 'US' },
    });
  ...
  it('serves research topic summaries and regime diagnostics', async () => {
    ...
    const topicRes = await requestLocalHttp(app, {
      path: '/api/research/topic',
      query: { topic: 'momentum' },
    });
    ...
    const regimeRes = await requestLocalHttp(app, {
      path: '/api/research/diagnostics/regime',
      query: {
  ...
  it('serves evaluation, workflow, and experiment registry endpoints', async () => {
    ...
    const evaluationRes = await requestLocalHttp(app, {
      path: '/api/research/evaluation/strategy',
    });
    expect(evaluationRes.status).toBe(200);
    expect(evaluationRes.body).toHaveProperty('report');

    const workflowRes = await requestLocalHttp(app, {
      path: '/api/research/workflow',
      query: { topic: 'momentum' },
    });
    expect(workflowRes.status).toBe(200);
    expect(workflowRes.body.workflow?.stages?.length).toBeGreaterThanOrEqual(5);

    const experimentsRes = await requestLocalHttp(app, { path: '/api/research/experiments' });
