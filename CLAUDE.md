...
- `npm run verify` -- sequential lint + `format:check` + typecheck + test + root `build` + `build:landing` + `build:admin` (must pass before marking work done)
