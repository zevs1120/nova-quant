...
- **refactor(api,queries):** 将公开 Signals API Key 校验抽到 `queries/publicSignalsApiKey.ts`（工厂注入 `getRepo` / `tryPrimaryPostgresRead`）；将执行笔记解析、路由与 `deriveSignalNotional` 抽到 `queries/executionLiveNotes.ts`；将执行对账与 kill-switch 治理核心抽到 `queries/executionGovernanceCore.ts`（`queries.ts` 通过 `runExecutionGovernance` 注入 `getLiveOrderStatus`）；`getMarketModules` 改为直接调用 `buildMarketModulesFromRows`，去掉重复映射逻辑。
...
- **feat(alpha): 把公开研究线索接入 alpha discovery 证据链。** 新增 volatility-managed TSMOM、52-week high anchor、funding-confirmed crypto momentum、BTC-anchor crypto reversion 四组 public seed；seed runtime 会保留 public reference URL，alpha evaluator 会把 hypothesis/template/source URL 写入 evaluation metrics 与 `alpha_discovery_research_evidence` artifact，便于从论文线索一路追到候选、bar replay 与晋升判断。
...
### 📱 Today 首页改成容器优先的自适应布局

- **fix(today,ui): 重写 Today shell / deck 的响应式尺寸逻辑，修复窄屏浏览器里的右侧裁切与拥挤。**
- 顶部 Climate 区改成标题块 + 状态 pill 的弹性布局，不再把状态点硬顶到最右侧，避免在手机浏览器里被裁掉。
- Today 卡片列表不再依赖脆弱的固定卡高和大幅负重叠公式；改为自适应滚动列表，窄屏下标题、Watchlist 按钮和右侧 pill 会自动换行/收缩。
- Action card 在小屏设备上会自动放宽 footer 宽度、允许 validity pill 换行、并根据屏宽重排执行信息，确保任何设备都能完整显示主要内容。
...
- **Commit 测试**：执行了一次例行的 commit 操作以验证提交流程。
