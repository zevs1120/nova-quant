...
全量测试默认 **并行**（勿无故改回单线程串行，以免 `npm test` 从约 10s 级恶化到 50s+）。前端主壳为 JSX 时，优先为 **`src/utils` 关键路径**（如 `fetchApi` 本地 base 轮换、`src/shared/http/apiGovernance.js` 与 `fetchApi` 集成、`appHelpers`、`format`、`firstRunRouting`、`signalHumanLabels`、`signalEntryBounds`）补充 Node 侧单测；会员裁剪见 `tests/membershipDecisionAccess.test.ts`；**Hooks** 使用 `happy-dom` + `@testing-library/react` 的 `renderHook`（见 `tests/hooks/`），**admin 组件**见 `tests/admin/`。策略与说明见 `docs/TESTING.md`（主壳/Today/Nova 相关索引表）。
