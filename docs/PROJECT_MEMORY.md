...
Last updated: 2026-03-09 (Asia/Shanghai)
...
1. Backtest realism and paper/backtest consistency still need deeper correction.
2. Shadow forward path remains synthetic and needs bar-joined realized path.
3. Cost/slippage models remain simplified (venue-specific calibration pending).
4. Governance reviewer workflow is still system-generated by default.
5. Discovery validation is still simulation-proxy based and should be upgraded to full market replay for production claims.
6. Portfolio simulation currently uses deterministic proxy dynamics and should move to realized event-level replay.
