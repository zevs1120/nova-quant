...
- The main shell’s [src/utils/api.js](src/utils/api.js) wraps all product `fetchApi` traffic with [src/shared/http/apiGovernance.js](src/shared/http/apiGovernance.js): safe-request in-flight dedupe, per-route minimum spacing, failure backoff, cross-tab shared cooldown state, and a short global pause when Vercel returns `402` + `X-Vercel-Error: DEPLOYMENT_DISABLED`. In-flight dedupe is intentionally limited to **GET / HEAD** plus the explicit idempotent read `POST /api/engagement/state`; write actions still execute one request per user action.
