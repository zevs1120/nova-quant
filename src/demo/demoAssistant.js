...
function holdingsRows(state) {
  return Array.isArray(state?.user_context?.holdings_review?.rows)
    ? state.user_context.holdings_review.rows
    : [];
}
...
function buildEvidenceLine(signal, state, language = 'en') {
  ...
  const entryHigh = safeNumber(
    signal?.entry_zone?.high ?? signal?.entry_high ?? signal?.entry,
    null,
  );
  ...
  const safety =
    state?.safety?.conclusion ||
    (zh ? '当前只能用小仓位控制风险。' : 'Risk is controlled only with small size.');
  ...
  return [
    ...
    confidence
      ? zh
        ...
        : `Confidence ${Math.round(confidence * 100)} out of 100.`
      : null,
    zh
      ? 'Demo 说明：这条回答来自离线投资者演示流程，不是实时助手。'
...
export function buildDemoAssistantReply(question, state, context = {}) {
    ...
    return structured({
      ...
      plan: biggest
        ...
        : zh
          ...
          : [
              'Load the investor demo holdings list.',
              'Keep total exposure moderate.',
              'Avoid stacking similar positions.',
            ],
      why: zh
        ? [
            '这个 demo 组合是为了看起来真实，而不是为了过度优化。',
            '产品想展示的是纪律和清晰度，不是高换手。',
            '仓位建议保持简单，方便第一次使用的投资者理解。',
          ]
        : [
    ...
    return structured({
      ...
      why: zh
        ? [
            ...
            signal
              ? `${signal.symbol} 会被展示，是因为它是今天 demo 包里最干净的 setup。`
              : '当前没有活跃的 demo 信号。',
          ]
        : [
            ...
            signal
              ? `${signal.symbol} is shown because it is the cleanest setup in the demo pack today.`
              : 'There is no active demo signal right now.',
          ],
      risk: zh
  ...
  if (intent === 'entry') {
    const entryLow = safeNumber(
      signal?.entry_zone?.low ?? signal?.entry_low ?? signal?.entry,
      null,
    );
    const entryHigh = safeNumber(
      signal?.entry_zone?.high ?? signal?.entry_high ?? signal?.entry,
    ...
    return structured({
      ...
      plan: zh
        ...
        : [
            ...
            tp !== null
              ? `Take profit is ${formatPrice(tp)}.`
              : 'Take profit comes first; do not freestyle the target.',
      ...
      why: zh
        ? [
            '这个 demo 想展示的是先有计划，再去行动。',
            '入场、目标和止损被一起展示，是为了让交易一眼能看懂。',
            '重点是受控执行，而不是最大暴露。',
          ]
        : [
      ...
      risk: zh
        ? ['如果价格已经明显偏离区间，正确动作就是继续等。']
        : ['If price moves too far from the zone, the right choice is to wait.'],
      evidence: [primaryEvidence],
    });
  }
  ...
  if (intent === 'strategy') {
    return structured({
      language,
      verdict: zh
        ? 'Nova 先看证据，再决定是否给出小仓位动作。'
        : 'Nova uses a simple evidence-first process, then keeps size small.',
      plan: zh
        ? [
            '先找当前面板里最干净的 setup。',
            '再看市场是否足够平稳，允许动作。',
            '最后整理成一张包含入场、目标、止损和仓位的行动卡。',
          ]
        : [
      ...
      why: zh
        ? [
            `${strategySource} 是 demo 卡片上展示的策略标签。`,
            '这套体验是给散户用户设计的，所以先解释动作，再展开细节。',
            '目标是减少冲动交易，而不是把产品做得很花哨。',
          ]
        : [
  ...
  if (intent === 'signal') {
    return structured({
      ...
      why: zh
        ? [
            'demo 的目标是给你一个强动作，而不是十个噪音想法。',
            '相比完整交易后台，一张清晰行动卡更容易让投资者理解。',
            signal
              ? `${signal.symbol} 通过了 demo 的筛选规则，所以排在最前。`
              : '当前没有 setup 通过 demo 筛选。',
          ]
        : [
            ...
            signal
              ? `${signal.symbol} passed the demo selection rules and is shown first.`
              : 'No setup passed the demo selection rules.',
          ],
      risk: zh
  ...
  return structured({
    ...
    why: zh
      ? [
          'demo 助手故意保持简单，对投资者更友好。',
          '它故意做成离线，这样没有网络时演示流程也稳定。',
          context?.page
            ? `这条回答锚定在 ${context.page} 页面上下文。`
            : '这条回答锚定在当前 demo 上下文。',
        ]
      : [
          ...
          context?.page
            ? `This answer is anchored to the ${context.page} screen context.`
            : 'This answer is anchored to the current demo context.',
        ],
    risk: zh
      ? ['这是 demo 演示流程，不是实时交易会话。']
      : ['This is a demo walkthrough, not a live trading session.'],
