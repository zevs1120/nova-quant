      ...
      Object.entries(candidate.params).map(([key, value]) => {
        if (typeof value === 'number' && Number.isFinite(value)) {
          return [
            key,
            {
              min: round(value * 0.8, 4),
              max: round(value * 1.2, 4),
              step: Math.max(0.01, Math.abs(value) * 0.05),
            },
          ];
        }
...
function buildValidationContext(repo: MarketRepository) {
  const activeStrategies = repo.listStrategyVersions({ limit: 60 }).map((row) => ({
    family: row.family,
    current_stage:
      row.status === 'champion' || row.status === 'active'
        ? 'PROD'
...
function correlationToActive(repo: MarketRepository, candidate: AutonomousAlphaCandidate) {
  ...
  const liveFamilies = new Set(
    activeSignals.map((row) => String(row.strategy_family || '').toLowerCase()).filter(Boolean),
  );
...
function concentrationScore(validationRow: ValidationRow) {
  ...
  const windows = Array.isArray(walkForward?.metrics?.windows)
    ...
    : [];
  const positive = windows
    .map((row) => safeNumber(row.test_return, 0))
...
function buildAlphaMetrics(
  ...
  const winRate = Number.isFinite(Number(quick?.metrics?.win_rate))
    ? round(safeNumber(quick?.metrics?.win_rate, 0), 4)
    : null;
  const maxDrawdown = Number.isFinite(Number(quick?.metrics?.drawdown))
    ? round(safeNumber(quick?.metrics?.drawdown, 0), 4)
    : null;
  const sharpe = Number.isFinite(Number(quick?.metrics?.sharpe_proxy))
    ? round(safeNumber(quick?.metrics?.sharpe_proxy, 0), 4)
    : null;
  const plus25 = robust?.metrics?.cost_stress
    ? safeNumber((robust.metrics.cost_stress as Record<string, unknown>).plus_25pct_cost, 0)
    : null;
  const plus50 = robust?.metrics?.cost_stress
    ? safeNumber((robust.metrics.cost_stress as Record<string, unknown>).plus_50pct_cost, 0)
    : null;
  const strictFill = robust?.metrics?.cost_stress
    ? safeNumber((robust.metrics.cost_stress as Record<string, unknown>).strict_fill, 0)
    : null;
  ...
  return {
    ...
    turnover: Number.isFinite(Number(quick?.metrics?.turnover))
      ? round(safeNumber(quick?.metrics?.turnover, 0), 4)
      : null,
    cost_sensitivity: {
...
function buildRejectionReasons(
  ...
  if (metrics.correlation_to_active >= config.correlationRejectThreshold)
    reasons.push('too_correlated_with_active_alpha');
  if (
    (metrics.cost_sensitivity.plus_50pct_cost ?? 0) <= 0 ||
    (metrics.cost_sensitivity.strict_fill ?? 0) <= 0
  ) {
    reasons.push('performance_disappears_after_cost_assumptions');
  }
  if (metrics.concentration_score >= 0.68)
    reasons.push('performance_concentrated_in_one_short_subperiod');
...
function acceptanceScore(
  ...
  const costPenalty =
    (metrics.cost_sensitivity.plus_50pct_cost ?? 0) < 0
      ? 0.18
      : (metrics.cost_sensitivity.plus_50pct_cost ?? 0) < 0.01
        ? 0.08
        : 0;
...
function buildDefaultDatasetVersion(
  repo: MarketRepository,
  candidate: AutonomousAlphaCandidate,
): DatasetVersionRecord {
...
function buildDefaultUniverseSnapshot(
  repo: MarketRepository,
...
function persistBacktestProxy(
  ...
  const metric: BacktestMetricRecord = {
    ...
    realism_grade:
      evaluationStatus === 'PASS' ? 'B' : evaluationStatus === 'WATCH' ? 'C' : 'WITHHELD',
    robustness_grade:
...
export function evaluateAlphaCandidates(args: {
  ...
  const validationById = new Map(
    (validation.candidates as ValidationRow[]).map((row) => [row.candidate_id, row]),
  );
  const scoringById = new Map(
    (scoring.candidates as ScoringRow[]).map((row) => [row.candidate_id, row]),
  );
  ...
  for (const candidate of args.candidates) {
    ...
    const acceptedScore = acceptanceScore(
      scoringRow.candidate_quality_score,
      metrics,
      rejectionReasons,
    );
    const evaluationStatus: AlphaEvaluationRecord['evaluation_status'] =
      rejectionReasons.length > 0
        ? 'REJECT'
        : acceptedScore >= args.config.minAcceptanceScore
          ? 'PASS'
          : 'WATCH';
    const backtestRunId = persistBacktestProxy(
      args.repo,
      candidate,
      metrics,
      evaluationStatus,
