...
function pickSignal(args: ResearchToolArgs): SignalContract | null {
  ...
  if (args.symbol) {
    return rows.find((row) => String(row.symbol).toUpperCase() === String(args.symbol).toUpperCase()) || null;
  }
  return rows.find((row) => ['NEW', 'TRIGGERED'].includes(String(row.status).toUpperCase())) || rows[0] || null;
}
...
function supportingAndOpposing(signal: SignalContract | null, regimeTag: string | null) {
  ...
  return {
    supporting,
    opposing: [...new Set(opposingIds)].map((id) => getFactorDefinition(id)).filter(Boolean) as FactorCard[],
    regimeTag
  };
}
...
function latestBacktestDetail(runId?: string) {
  const repo = getRepo();
  const run = runId
    ? repo.getBacktestRun(runId)
    : repo.listBacktestRuns({ limit: 1 })[0];
  if (!run) {
    return {
...
function artifactMap(detail: ReturnType<typeof latestBacktestDetail>['detail']) {
  const entries = (detail?.artifacts || []).map(
    (row): [string, unknown] => [row.artifact_type, row.payload]
  );
...
export function getRegimeDiagnosticsTool(args: ResearchToolArgs) {
  ...
  return {
    source_status: normalizeRuntimeStatus(summary.runtime?.source_status, RUNTIME_STATUS.INSUFFICIENT_DATA),
    data_status: summary.rows.length ? RUNTIME_STATUS.MODEL_DERIVED : RUNTIME_STATUS.INSUFFICIENT_DATA,
    diagnostics: {
...
export function runFactorDiagnosticsTool(args: ResearchToolArgs) {
  ...
  return {
    source_status: normalizeRuntimeStatus(String(signalRow?.source_status || ''), RUNTIME_STATUS.DB_BACKED),
    data_status: normalizeRuntimeStatus(String(signalRow?.data_status || ''), RUNTIME_STATUS.MODEL_DERIVED),
    diagnostics: {
      signal_id: signal.id,
      symbol: signal.symbol,
      thesis: String(signalRow?.thesis || signal.strategy_id || signal.strategy_family || 'Signal exists because it cleared the current selection rules.'),
      ...
      regime_context: regime.regime_id,
      data_quality: normalizeRuntimeStatus(String(signalRow?.data_status || ''), RUNTIME_STATUS.MODEL_DERIVED),
      ...
      next_action: 'Validate the setup with replay/paper evidence before treating it as a production-quality edge.'
    }
  };
}
...
function familiesForFactor(factorId?: string) {
  ...
  if (key === 'low_vol' || key === 'quality' || key === 'value') return ['Relative Strength', 'Regime Transition'];
...
export function getBacktestIntegrityReportTool(args: ResearchToolArgs) {
  ...
  if ((metric.sharpe || 0) > 3 && (metric.sample_size || 0) < 25) flags.push('possible_overfit_proxy');
...
export function explainWhyNoSignalTool(args: ResearchToolArgs) {
  ...
  const active = rows.filter((row) => ['NEW', 'TRIGGERED'].includes(String(row.status).toUpperCase()));
  const regime = currentRegimeSummary(args);
  if (active.length) {
    return {
      source_status: RUNTIME_STATUS.DB_BACKED,
  ...
  return {
    source_status: normalizeRuntimeStatus(regime.runtime?.source_status, RUNTIME_STATUS.INSUFFICIENT_DATA),
    data_status: RUNTIME_STATUS.MODEL_DERIVED,
    explanation: {
      reason: 'No candidate cleared the current combination of signal quality, regime fit, and risk constraints.',
      current_regime: regime.regime_id,
      likely_filters: ['quality threshold', 'regime mismatch', 'risk posture'],
      next_action: 'Inspect signal funnel diagnostics or loosen constraints only after evidence review.'
    }
  };
}
...
export function listFailedExperimentsTool() {
  ...
  const records = (recent.records || [])
    ...
    .map((row: Record<string, unknown>, index: number) => ({
      ...
      title: String((row.strategy as Record<string, unknown> | undefined)?.strategy_key || row.run_type || 'unknown_run'),
      domain: 'backtest',
      likely_causes: [
        Number((row.metrics as Record<string, unknown> | undefined)?.net_return || 0) < 0 ? 'negative_cost_adjusted_return' : 'withheld_result',
        Number((row.metrics as Record<string, unknown> | undefined)?.turnover || 0) > 35 ? 'high_turnover' : 'sample_or_quality_constraint'
      ],
