...
const FEATURE_REGISTRY: FeatureSpec[] = [
  ...
  },
  {
    ...
    category: 'market_state',
    description: 'Normalizes stretch and extension so risk posture can de-escalate crowded conditions.',
    point_in_time_safe: true,
    online_serving_ready: true,
    serving_keys: ['market', 'symbol', 'timeframe', 'snapshot_ts_ms']
  },
  ...
  },
  {
    ...
    category: 'portfolio',
    description: 'Measures user portfolio overlap with the suggested action before personalization.',
    point_in_time_safe: true,
    online_serving_ready: true,
  ...
  },
  {
    ...
    category: 'execution',
    description: 'Execution realism proxy used across backtest, replay, paper, and decision cautioning.',
    point_in_time_safe: true,
  ...
  },
  {
    ...
    description: 'Validation gate used to prevent stale bars from surfacing as actionable intelligence.',
    point_in_time_safe: true,
    online_serving_ready: true,
...
export function buildFeaturePlatformSummary(
  ...
  const validationGates: ValidationResult[] = [
    ...
    },
    {
      ...
      notes: ['Feature and decision cache keys are isolated by user/risk/market/context to avoid serving skew.']
    }
  ];
  ...
  return {
    ...
    point_in_time_contract: {
      ...
      training_serving_skew_policy: 'Feature specs define the same serving keys for offline and online access.',
      serving_keys_required: ['user_id', 'market', 'asset_class', 'strategy_context', 'snapshot_ts_ms']
    },
    offline_online_parity: {
      parity_status: datasets.length && featureSnapshots.length ? 'PARTIAL_READY' : 'SCAFFOLD_READY',
      note:
        'Dataset versions and feature snapshots are stored canonically; some runtime features are still derived on demand and should migrate into snapshot-backed serving over time.'
    },
    ...
    cache_isolation_dimensions: ['user_id', 'risk_profile_key', 'market', 'asset_class', 'strategy_context', 'snapshot_date']
  };
}
