...
function workflowSummaryForRow(row: WorkflowRunRecord): JsonObject | null {
  ...
  if (row.workflow_key === 'alpha_discovery_loop') {
    const evaluationSummary =
      output.evaluation_summary && typeof output.evaluation_summary === 'object'
        ...
        : null;
    const alphaRegistry = output.alpha_registry && typeof output.alpha_registry === 'object' ? (output.alpha_registry as JsonObject) : null;
    return {
      accepted: evaluationSummary?.accepted ?? null,
      rejected: evaluationSummary?.rejected ?? null,
      watch: evaluationSummary?.watch ?? null,
      top_candidates:
        Array.isArray(alphaRegistry?.top_candidates) ? (alphaRegistry?.top_candidates as unknown[]).length : null
    };
  }
  if (row.workflow_key === 'alpha_shadow_runner') {
    const shadow = output.shadow && typeof output.shadow === 'object' ? (output.shadow as JsonObject) : null;
    const promotion = output.promotion && typeof output.promotion === 'object' ? (output.promotion as JsonObject) : null;
    return {
      candidates_processed: shadow?.candidates_processed ?? null,
      promoted_to_canary: Array.isArray(promotion?.promoted_to_canary) ? (promotion.promoted_to_canary as unknown[]).length : null,
  ...
  if (row.workflow_key === 'nova_training_flywheel') {
    const execution = output.execution && typeof output.execution === 'object' ? (output.execution as JsonObject) : null;
    return {
...
function summarizeTrainingRun(row: WorkflowRunRecord): TrainingRunSummary {
  ...
  return {
    ...
    task_types: Array.isArray(output.task_types) ? output.task_types.map((item) => String(item)) : [],
    execution: {
...
function getTimeZoneParts(date: Date, timeZone: string) {
  const formatted = getTimeZoneFormatter(timeZone).formatToParts(date).reduce<Record<string, string>>((acc, part) => {
...
function getStartOfDayUtcMs(timeZone: string, localDate?: string) {
  const normalizedDate =
    typeof localDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(localDate) ? localDate : getTimeZoneDateKey(new Date(), timeZone);
...
function queryTableCount(db: ReturnType<typeof getDb>, table: string, column: string, sinceMs: number): DailyTableCount {
  const row = db
    .prepare(`SELECT COUNT(*) AS count, MAX(${column}) AS last_ms FROM ${table} WHERE ${column} >= @since_ms`)
...
function buildLocalSnapshot(args?: { timeZone?: string; localDate?: string }): AdminResearchOpsSnapshot {
  const repo = getRepo();
  const db = getDb();
  ensureSchema(db);
  ...
  alphaEvaluations.forEach((row) => {
    ...
    const netPnl =
      Number.isFinite(Number(metrics.net_pnl)) ? Number(metrics.net_pnl) : Number.isFinite(Number(metrics.net_return)) ? Number(metrics.net_return) : null;
  ...
  const alphaEvalSummary = Array.from(alphaEvalMap.values())
    .map((row) => ({
      ...
      max_acceptance: row.max_acceptance > Number.NEGATIVE_INFINITY ? round(row.max_acceptance, 4) : 0,
      avg_net_pnl: row.net_pnl_values.length ? round(row.net_pnl_values.reduce((sum, value) => sum + value, 0) / row.net_pnl_values.length, 4) : null,
      best_net_pnl: row.net_pnl_values.length ? round(Math.max(...row.net_pnl_values), 4) : null,
      avg_sharpe: row.sharpe_values.length ? round(row.sharpe_values.reduce((sum, value) => sum + value, 0) / row.sharpe_values.length, 4) : null
    }))
    .sort((a, b) => orderEvaluationStatus(a.evaluation_status) - orderEvaluationStatus(b.evaluation_status));
  ...
  const topBacktests = repo
    ...
    .sort((a, b) => Number(b.net_return || -Infinity) - Number(a.net_return || -Infinity) || Number(b.sharpe || -Infinity) - Number(a.sharpe || -Infinity))
    .slice(0, 8);
  ...
  const factorCoveragePct = recentNewsItems.length ? round((ops.recent_news_factors.length / recentNewsItems.length) * 100, 1) : 0;
  ...
  return {
    ...
    workflow_summary: {
      ...
      by_status: workflowSummaryTotal ? countBy(recentWorkflows, (row) => row.status) : countBy(ops.workflows, (row) => row.status),
      by_workflow: workflowSummaryTotal ? countBy(recentWorkflows, (row) => row.workflow_key) : countBy(ops.workflows, (row) => row.workflow_key)
    },
...
function resolveLiveApiBase() {
  ...
  return candidates.map((value) => String(value || '').trim().replace(/\/+$/, '')).find(Boolean) || '';
}

function resolveLiveApiToken() {
  return String(process.env.NOVA_ADMIN_LIVE_API_TOKEN || process.env.NOVA_CONTROL_PLANE_API_TOKEN || '').trim();
}

function resolveReportTimeZone(explicitTimeZone?: string) {
  return String(explicitTimeZone || process.env.NOVA_ADMIN_REPORT_TIMEZONE || 'Asia/Shanghai').trim() || 'Asia/Shanghai';
}
...
async function fetchUpstreamSnapshot(args?: { timeZone?: string; localDate?: string }) {
  ...
  const timeZone = resolveReportTimeZone(args?.timeZone);
  const localDate = typeof args?.localDate === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(args.localDate) ? args.localDate : '';
...
export function buildLocalAdminResearchOpsSnapshot(args?: { timeZone?: string; localDate?: string }) {
  return buildLocalSnapshot(args);
}

export async function buildAdminResearchOpsSnapshot(args?: { timeZone?: string; localDate?: string }) {
