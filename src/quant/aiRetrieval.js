...
function isActionable(signal) {
  ...
  return (
    ACTIVE_SIGNAL_STATUS.has(status) && !['WITHHELD', 'INSUFFICIENT_DATA'].includes(dataStatus)
  );
}
...
function pickTopSignal(state, ticker) {
  const evidenceSignals = Array.isArray(state?.evidence?.top_signals)
...
function sourceNote(state, signal) {
  ...
  if (dataStatus === 'WITHHELD')
    return 'The system is holding this back because the sample is too weak.';
...
function explainStrategySimply(signal, state) {
  ...
  return {
    ...
    why: [
      ...
      panda.topFactors.length
        ? `Today the strongest checks are ${panda.topFactors.join(', ')}.`
        : 'Today the engine is using its standard price and safety checks.',
...
function buildPayload(intent, state, ticker) {
  ...
  if (intent === 'holdings_risk') {
    ...
    return {
      verdict:
        holdingsReview.key_advice || 'Your portfolio needs a risk cleanup before new trades.',
      plan: [
        riskiest
          ? `Start with ${riskiest.symbol}. That is the first position to review.`
          : 'Start with your biggest position first.',
      ...
      why: [
        ...
        (holdingsReview.risk?.primary_risks || [])[0] ||
          'The system sees concentration risk in your current mix.',
    ...
    return {
      ...
      plan: [
        risk.label === 'Dangerous'
          ? 'Stay defensive and keep cash first.'
          : `If you act, keep size near ${position} only.`,
        'Use the stop line. Do not widen it after entry.',
        'If the setup feels unclear, waiting is a valid decision.',
      ],
      why: [`The system posture today is ${posture}.`, reasons[1], reasons[2]],
      risk: [risk.note, 'The goal is to avoid bad trades, not to trade every day.'],
      evidence: [
        sourceNote(state, signal),
        signal ? `Signal freshness: ${freshness}.` : 'No active signal right now.',
      ],
    };
  }
  ...
  if (intent === 'buy_or_sell') {
    if (!signal) {
      return {
        ...
        plan: [
          'Keep your watchlist ready, but do not force a trade.',
          'Come back when a clean setup appears.',
        ],
        why: [
          'No clear signal is at the top of the list.',
          'Safety comes before activity.',
          sourceNote(state, signal),
        ],
    ...
    return {
      ...
      plan: [
        ...
        target !== null
          ? `Take profit around ${target}.`
          : 'Take profit only if price moves cleanly in your favor.',
        stop !== null
          ? `Step out if price falls to ${stop}.`
          : 'Use a clear stop before you enter.',
      ],
      why: [reasons[0], reasons[1], `This signal was updated ${freshness}.`],
      risk: [
  ...
  if (intent === 'why_signal') {
    return {
      ...
      plan: [
        signal
          ? `Treat it as a small, planned trade near ${position}.`
          : 'Stay patient and let the next clean setup come to you.',
        signal && target !== null
          ? `Take profit around ${target}.`
          : 'Only act when entry, stop, and target are all clear.',
        signal && stop !== null ? `Stop loss around ${stop}.` : 'Keep risk small from the start.',
      ].filter(Boolean),
      why: reasons,
      risk: ['A clear signal is still not a guarantee.', risk.note],
      evidence: [
        sourceNote(state, signal),
        signal ? `Signal freshness: ${freshness}.` : 'No signal is active.',
      ],
    };
  }
  ...
  return {
    ...
    plan: [
      signal
        ? `If you follow the plan, keep it near ${position}.`
        : 'Wait for the next clean chance.',
      'Check risk first, then decide.',
      'If something feels rushed, skip it.',
    ],
    why: [`The current market posture is ${posture}.`, reasons[0], reasons[2]],
    risk: [risk.note, 'The app is designed to help you stay disciplined, not busy.'],
    evidence: [sourceNote(state, signal)],
  };
}
...
function detectIntent(question) {
  ...
  if (text.includes('safe') || text.includes('risk') || text.includes('安全')) {
    return 'safe_check';
  }
  if (
