  ...
  return {
    ...
    fetchRawSnapshot({ asOf, productIds = DEFAULT_PRODUCTS } = {}) {
      ...
      const selected = sample.instruments.filter(
        (item) => item.market === 'CRYPTO' && productIds.includes(item.ticker),
      );
      ...
      const bars = selected.flatMap((item) => {
        ...
        return item.bars.slice(-140).map((bar, index) => ({
          ...
          trades_count: Math.max(
            12,
            Math.round(
              (bar.volume / Math.max(bar.close, 1)) * 0.9 +
                deterministicNoise(seed, index + 9) * 40,
            ),
          ),
      ...
      return {
        metadata: sourceMeta({
          source:
            mode === 'live_path_available' ? 'coinbase_public' : 'local_sample_crypto_adapter',
