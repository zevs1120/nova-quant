...
export const NOVA_BRAND_VOICE = {
  en: {
    ...
    opportunity:
      'Speak with clarity and restraint. Opportunity is permission to focus, not permission to get excited.',
    risk: 'Name the boundary plainly. Risk language should feel like a steady hand, not a siren.',
    doNotAct:
      'Frame no-action as intelligent capital preservation and decision quality, not absence.',
    returnInvite:
      'Invite the user back for a clean check-in because the judgment moved or deserves confirmation.',
    noActionValue: 'No-action is often a premium decision. Say that directly and with composure.',
    impulsiveIntervention:
      'Sound like you are putting a hand on the user’s shoulder, not wagging a finger.',
...
const COPY_LIBRARY = {
  en: {
    ...
    morningHumor: {
      ...
      DEFEND: [
        'If you are itching to do something, the market may be better at teaching than forgiving.',
      ],
    ...
    perception: {
      ...
      shifted: {
        ATTACK: ['The view sharpened. Take the updated read, not the mood you woke up with.'],
        PROBE: ['The tone moved. A fresh confirmation matters more than a faster reaction.'],
        DEFEND: [
          'The line moved. The useful action now is to notice that before you do anything else.',
        ],
        WAIT: ['The day changed just enough to deserve another clean look, not a louder response.'],
      },
      anchored: {
        ATTACK: [
          'Today’s view is already anchored. You do not need to keep asking the market for permission.',
        ],
        PROBE: ['The day is already calibrated. Let discipline do the rest of the work.'],
        DEFEND: [
          'Today is already anchored around restraint. That is a position, not an omission.',
        ],
        WAIT: ['The quiet answer is already locked in. Nothing else needs to be forced into it.'],
      },
      focus: {
        actionable: {
          ATTACK: [
        ...
        noAction: {
          ATTACK: ['Clarity improved, but not enough to earn a louder move.'],
          PROBE: ['The useful outcome today is that nothing earned the right to drag you around.'],
          DEFEND: [
            'Today’s cleanest result is that the system filtered out the unnecessary urge to do more.',
          ],
          WAIT: ['The market still owes you a better reason than motion alone.'],
        },
      },
    },
    noAction: {
    ...
    actionCard: {
      ...
      viewExplain: ['See why this card is here', 'Open the evidence behind this card'],
    },
    widget: {
      state: {
        ATTACK: {
    ...
    notifications: {
      ...
      STATE_SHIFT: {
        ...
        body: {
          risk_shift: [
            'The risk posture moved. Re-check before carrying the earlier tone forward.',
          ],
      ...
      PROTECTIVE: {
        ...
        body: {
          ...
          overlap: [
            'Your current exposure already overlaps enough. New action deserves a harder filter.',
          ],
        },
      },
      WRAP_UP: {
    ...
    assistant: {
      opener: {
        ...
        protective: [
          'Let’s not give noise more authority than it deserves.',
          'Before we touch the idea, let’s check the boundary.',
        ],
  ...
  zh: {
    ...
    todayRisk: {
      ATTACK: {
        label: '风险回落',
        explanation: ['今天可以有选择地动作。'],
        deltaUp: ['风险比昨天轻一点，但仍然不需要激进。'],
        deltaFlat: ['风险仍可控，仓位依旧属于纪律的一部分。'],
      },
      PROBE: {
        label: '风险偏混合',
        explanation: ['今天是选择日，不是全场绿灯。'],
        deltaUp: ['风险回落了一点，但还不到可以放松。'],
        deltaFlat: ['气候仍偏混合，门槛继续放高。'],
      },
      DEFEND: {
        label: '风险偏高',
        explanation: ['今天更需要边界，而不是新风险。'],
        deltaUp: ['风险进一步抬高，先守线。'],
        deltaFlat: ['风险压力还在，你不需要强行给市场答案。'],
      },
      WAIT: {
        label: '清晰度不足',
        explanation: ['今天更适合验证，不适合高风险动作。'],
        deltaUp: ['清晰度还没好到值得动作。'],
        deltaFlat: ['边还太软，不值得靠过去。'],
      },
    },
    morningCheck: {
      PENDING: {
        title: 'Morning Check',
        shortLabel: '待确认',
        headline: ['今天的判断已经到了，先确认。'],
        prompt: ['不用研究很久，只要确认今天该不该动。'],
        completion: ['你今天最重要的动作，是先确认判断。'],
        cta: '确认今天判断',
        aiCta: '为什么这样看',
      },
      REFRESH_REQUIRED: {
        title: 'Morning Check',
        shortLabel: '已更新',
        headline: ['判断变了，值得再看一眼。'],
        prompt: ['系统更新了判断，先确认变化，再决定要不要动作。'],
        completion: ['先重新校准，再延续今天的动作。'],
        cta: '重新确认',
        aiCta: '哪里变了',
      },
      COMPLETED: {
        ...
        shortLabel: '已确认',
        headline: ['今天的判断已经确认。'],
        prompt: ['最重要的动作已经完成，接下来只剩执行纪律。'],
        completion: ['今天已经锚定，不需要反复拧紧它。'],
        cta: '今日已记下',
        aiCta: '解释判断',
      },
    },
    ...
    perception: {
      ...
      arriving: {
        ...
        PROBE: ['系统已经先筛出今天那条窄边。你要做的是确认，不是把它脑补成全场放行。'],
        DEFEND: [
...
export function getDailyStanceCopy({
  posture,
  locale = 'en',
  variant = 'standard',
  seed = '',
  ...
  const selected = choose(
    bucket[variant] || bucket.standard,
    `${posture}:${variant}:${seed}`,
    bucket.standard[0],
  );
  const adjusted =
    noActionDay && normalizePosture(posture) !== 'ATTACK'
...
export function getTodayRiskCopy({ posture, locale = 'en', changed = false, seed = '' }) {
  ...
  return {
    label: bucket.label,
    explanation: guard(
      choose(bucket.explanation, `${posture}:explain:${seed}`, bucket.explanation[0]),
      bucket.explanation[0],
    ),
    delta: guard(
      choose(
        changed ? bucket.deltaUp : bucket.deltaFlat,
        `${posture}:delta:${seed}`,
        (changed ? bucket.deltaUp : bucket.deltaFlat)[0],
      ),
      (changed ? bucket.deltaUp : bucket.deltaFlat)[0],
    ),
  };
...
export function getMorningCheckCopy({
  posture,
  status,
  locale = 'en',
  seed = '',
  ...
  const lib = library(locale);
  const key = ['PENDING', 'REFRESH_REQUIRED', 'COMPLETED'].includes(
  ...
  return {
    ...
    headline: guard(
      choose(bucket.headline, `${key}:headline:${seed}`, bucket.headline[0]),
      bucket.headline[0],
    ),
    prompt: guard(
      choose(bucket.prompt, `${key}:prompt:${seed}`, bucket.prompt[0]),
      bucket.prompt[0],
    ),
    arrival_line: guard(
      choose(
        lib.morningArrival[normalizePosture(posture)],
        `${posture}:arrival:${seed}`,
        lib.morningArrival[normalizePosture(posture)][0],
      ),
      '',
    ),
    ritual_line: guard(
      choose(
        lib.morningRitual[normalizePosture(posture)],
        `${posture}:ritual:${seed}`,
        lib.morningRitual[normalizePosture(posture)][0],
      ),
      '',
    ),
    humor_line: guard(
      choose(
        lib.morningHumor[normalizePosture(posture)],
        `${posture}:humor:${seed}`,
        lib.morningHumor[normalizePosture(posture)][0],
      ),
      '',
    ),
...
export function getActionCardCopy({
  posture,
  locale = 'en',
  seed = '',
  ...
  return {
    ...
    why_now: guard(
      choose(
        lib.actionCard.whyNow[normalized],
        `${normalized}:why:${seed}`,
        lib.actionCard.whyNow[normalized][0],
      ),
      lib.actionCard.whyNow[normalized][0],
    ),
    caution: guard(
      choose(
        lib.actionCard.caution[normalized],
        `${normalized}:caution:${seed}`,
        lib.actionCard.caution[normalized][0],
      ),
      lib.actionCard.caution[normalized][0],
    ),
    invalidation: guard(
      actionState === 'watch-only'
        ? choose(
            lib.actionCard.invalidation.watchOnly,
            `${normalized}:invalidation:${seed}`,
            lib.actionCard.invalidation.watchOnly[0],
          )
        : choose(
            lib.actionCard.invalidation.actionable,
            `${normalized}:invalidation:${seed}`,
...
export function getNoActionCopy({ locale = 'en', seed = '', posture = 'WAIT' }) {
  ...
  return {
    arrival: guard(
      choose(lib.noAction.arrival, `${posture}:arrival:${seed}`, lib.noAction.arrival[0]),
      lib.noAction.arrival[0],
    ),
    completion: guard(
      choose(lib.noAction.completion, `${posture}:complete:${seed}`, lib.noAction.completion[0]),
      lib.noAction.completion[0],
    ),
    wrap: guard(
      choose(lib.noAction.wrap, `${posture}:wrap:${seed}`, lib.noAction.wrap[0]),
      lib.noAction.wrap[0],
    ),
    notify: guard(
      choose(lib.noAction.notify, `${posture}:notify:${seed}`, lib.noAction.notify[0]),
      lib.noAction.notify[0],
    ),
  };
}
...
export function getNotificationCopy({
  ...
  const lib = library(locale);
  const bucket =
    lib.notifications[String(category || 'RHYTHM').toUpperCase()] || lib.notifications.RHYTHM;
  if (String(category || '').toUpperCase() === 'STATE_SHIFT') {
    return {
      title: guard(
        choose(bucket.title, `${category}:title:${seed}`, bucket.title[0]),
        bucket.title[0],
      ),
      body: guard(
        choose(
          bucket.body[triggerType] || bucket.body.stable,
          `${category}:body:${seed}`,
          (bucket.body[triggerType] || bucket.body.stable)[0],
        ),
        (bucket.body[triggerType] || bucket.body.stable)[0],
      ),
    };
  ...
  if (String(category || '').toUpperCase() === 'PROTECTIVE') {
    ...
    return {
      ...
      body: guard(
        choose(
          bucket.body[bodyKey] || bucket.body.DEFEND,
          `${category}:body:${seed}`,
          (bucket.body[bodyKey] || bucket.body.DEFEND)[0],
        ),
        (bucket.body[bodyKey] || bucket.body.DEFEND)[0],
      ),
    };
...
export function getWidgetCopy({ type, posture, locale = 'en', triggerType = 'stable', seed = '' }) {
  ...
  if (type === 'change') {
    return {
      title: guard(
        choose(
          lib.widget.change[triggerType] || lib.widget.change.stable,
          `${type}:title:${seed}`,
          (lib.widget.change[triggerType] || lib.widget.change.stable)[0],
        ),
        (lib.widget.change[triggerType] || lib.widget.change.stable)[0],
      ),
      spark: guard(
        choose(
          lib.widget.spark[normalizePosture(posture)],
          `${type}:spark:${seed}`,
          lib.widget.spark[normalizePosture(posture)][0],
        ),
        lib.widget.spark[normalizePosture(posture)][0],
      ),
    };
  ...
  return {
    title: guard(
      choose(
        lib.widget.state[normalizePosture(posture)].title,
        `${type}:title:${seed}`,
        lib.widget.state[normalizePosture(posture)].title[0],
      ),
      lib.widget.state[normalizePosture(posture)].title[0],
    ),
    caption: guard(
      choose(
        lib.widget.state[normalizePosture(posture)].caption,
        `${type}:caption:${seed}`,
        lib.widget.state[normalizePosture(posture)].caption[0],
      ),
      lib.widget.state[normalizePosture(posture)].caption[0],
    ),
    spark: guard(
...
export function getDisciplineCopy({ locale = 'en', score = 0, noActionDay = false, seed = '' }) {
  ...
  return {
    summary: guard(
      choose(lib.discipline[bucket], `${bucket}:summary:${seed}`, lib.discipline[bucket][0]),
      lib.discipline[bucket][0],
    ),
    no_action_value_line: noActionDay
      ? guard(
          choose(lib.discipline.noAction, `no-action:${seed}`, lib.discipline.noAction[0]),
          lib.discipline.noAction[0],
        )
      : null,
...
export function getWrapUpCopy({
  ...
  return {
    ...
    short_label: completed
      ? lib.wrapUp.shortDone
      : ready
        ? lib.wrapUp.shortReady
        : normalizeNovaLocale(locale) === 'zh'
          ? '稍后'
          : 'Later',
    headline: guard(
    ...
    opening_line: guard(
      choose(
        lib.wrapUp.opening[normalizePosture(posture)],
        `wrap:opening:${seed}`,
        lib.wrapUp.opening[normalizePosture(posture)][0],
      ),
      lib.wrapUp.opening[normalizePosture(posture)][0],
    ),
    completion_feedback: guard(
      choose(lib.wrapUp.completion, `wrap:complete:${seed}`, lib.wrapUp.completion[0]),
      lib.wrapUp.completion[0],
    ),
    no_action_line: noActionDay
      ? guard(
          choose(lib.wrapUp.noAction, `wrap:no-action:${seed}`, lib.wrapUp.noAction[0]),
          lib.wrapUp.noAction[0],
        )
      : null,
  };
}
...
export function getPerceptionLayerCopy({
  ...
  const lib = library(locale);
  const normalized = normalizePosture(posture);
  const statusKey = status === 'anchored' ? 'anchored' : changed ? 'shifted' : 'arriving';
  ...
  return {
    ...
    headline: guard(
      choose(bucket, `${normalized}:perception:${statusKey}:${seed}`, bucket[0]),
      bucket[0],
    ),
    focus_line: guard(
      choose(focusBucket, `${normalized}:perception:focus:${seed}`, focusBucket[0]),
      focusBucket[0],
    ),
...
export function getAssistantVoiceGuide({
  locale = 'en',
  posture = 'WAIT',
  userState = 'default',
} = {}) {
  ...
  const openerKey =
    tone === 'defensive' ? 'protective' : tone === 'opportunity' ? 'opportunity' : 'calm';
  return {
    opener: guard(
      choose(
        lib.assistant.opener[openerKey],
        `${openerKey}:${userState}`,
        lib.assistant.opener[openerKey][0],
      ),
      lib.assistant.opener[openerKey][0],
    ),
    risk_explain: guard(
      choose(lib.assistant.riskExplain, `risk:${userState}`, lib.assistant.riskExplain[0]),
      lib.assistant.riskExplain[0],
    ),
    intercept: guard(
      choose(lib.assistant.intercept, `intercept:${userState}`, lib.assistant.intercept[0]),
      lib.assistant.intercept[0],
    ),
    no_action: guard(
      choose(lib.assistant.noAction, `no-action:${userState}`, lib.assistant.noAction[0]),
      lib.assistant.noAction[0],
    ),
    wrap: guard(
      choose(lib.assistant.wrap, `wrap:${userState}`, lib.assistant.wrap[0]),
      lib.assistant.wrap[0],
    ),
    style_rules:
      normalizeNovaLocale(locale) === 'zh'
        ...
        : [
            'calm but not cold',
            'a little wit is fine, never cute',
            'on high-risk days sound like you are slowing the user down',
          ],
  };
}
...
export function getUiRegimeTone({ posture, locale = 'en' }) {
  ...
  return {
    ...
    wrap_line: getWrapUpCopy({
      locale,
      posture: normalized,
      ready: true,
      completed: false,
      seed: `${normalized}:ui`,
      noActionDay: normalized !== 'ATTACK',
    }).opening_line,
