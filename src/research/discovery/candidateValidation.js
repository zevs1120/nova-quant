...
function estimatedSignalCount(candidate) {
  ...
  const count = base * (1.45 - tightness) * (0.8 + Math.min(regimeCount, 4) * 0.08) * (6 / Math.max(2, horizon.avg));
  return Math.max(0, Math.round(count));
}

function estimatedTurnover(candidate) {
  const horizon = parseHorizonDays(candidate.expected_holding_horizon);
  const modePenalty = candidate.generation_mode === 'exploratory' ? 0.16 : candidate.generation_mode === 'regime_tuned' ? 0.1 : 0.05;
...
function candidateMarketKey(candidate = {}) {
  const classes = (candidate.supported_asset_classes || []).map((item) => String(item).toUpperCase());
...
function sanityStage(candidate, cfg = {}) {
  ...
  const leverageCap = Number(candidate.parameter_set?.leverage_cap ?? candidate.parameter_set?.max_leverage ?? 1);
  ...
  if (sensitivity === 'high' && turnover > 1.05) reasons.push('fill_realism_risk_under_cost_stress');
...
function quickBacktestStage(candidate, cfg = {}, context = {}) {
  ...
  const grossReturn = Number.isFinite(replayReturn)
    ? clamp(
        replayReturn * (0.9 + prior * 0.5) + 0.012 + prior * 0.085 + noise * 0.025 - turnover * 0.03,
        -0.3,
        0.45
      )
  ...
  const drawdown = Number.isFinite(replayDrawdown)
    ? clamp(replayDrawdown * (0.85 + (1 - prior) * 0.4) + Math.abs(noise) * 0.04 + turnover * 0.08, 0.03, 0.65)
  ...
  return {
    ...
    metrics: {
      ...
      slippage_bps: round((baselineAssumption.entry_slippage_bps + baselineAssumption.exit_slippage_bps) / 2, 4),
...
function robustnessStage(candidate, quickStage, cfg = {}) {
  ...
  const scenarioMetrics = scenarios.map((scenario) => {
    ...
    const returnAfterScenario = baseReturn - Math.max(0, cost - estimateCostDragPct({
  ...
  const scenarioById = Object.fromEntries(scenarioMetrics.map((item) => [item.scenario_id, item.return]));
...
function walkForwardStage(candidate, quickStage, robustness, cfg = {}) {
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    windows.push({
      ...
      drawdown: round(clamp(safeNumber(quickStage?.metrics?.drawdown, 0.2) + Math.abs(noise) * 0.06, 0.03, 0.65), 6)
    });
  }
...
function portfolioContributionStage(candidate, quickStage, robustness, research = {}, cfg = {}) {
  ...
  const familyNovelty = liveFamilies.some((item) => String(item || '').toLowerCase() === familyKey) ? 0.45 : 0.78;
  ...
  const diversificationScore = clamp(familyNovelty * 0.62 + regimeScore * 0.3 + (noise + 1) * 0.04, 0, 1);
  const drawdownRelief = clamp((safeNumber(quickStage?.metrics?.drawdown, 0.2) - 0.18) * -1 + 0.05, -0.08, 0.12);
  const independentAlpha = clamp(baseReturn * diversificationScore * 1.2 + noise * 0.01, -0.12, 0.22);
  const improvePortfolio = independentAlpha > -0.005 && (drawdownRelief > -0.03 || diversificationScore > 0.55);
...
function evaluateCandidate(candidate, context = {}, config = {}) {
  ...
  const stage5 = portfolioContributionStage(candidate, stage2, stage3, context.research, config.stage_5);
