...
function activeSignals(signals = []) {
  return signals.filter((item) => ACTIVE_SIGNAL_STATUS.has(String(item.status || '').toUpperCase()));
}
...
function stopDistancePct(signal) {
  ...
  const entryMid = entryLow && entryHigh ? (entryLow + entryHigh) / 2 : Number(signal.entry_price ?? 0);
  const stop = Number(signal.stop_loss?.price ?? signal.stop_loss_value ?? signal.stop_loss ?? 0);
  if (!Number.isFinite(entryMid) || entryMid <= 0 || !Number.isFinite(stop) || stop <= 0) return 0.8;
...
function correlationFootprint(signals = []) {
  ...
  for (const signal of signals) {
    const theme = signal.market === 'CRYPTO' ? 'crypto_core' : String(signal.sector || 'equity_core');
    const current = byTheme.get(theme) || { theme, exposure_pct: 0, symbols: [] };
    current.exposure_pct += Number(signal.position_advice?.position_pct ?? signal.position_size_pct ?? 0);
  ...
  const highest = rows.reduce(
    (best, row) => (row.exposure_pct > best.exposure_pct ? row : best),
    { theme: 'none', exposure_pct: 0, symbols: [] }
  );
...
function tradeDecision({
  ...
  const basePositionPct = Number(signal.position_advice?.position_pct ?? signal.position_size_pct ?? 0);
  ...
  const recommendedPositionPct = round(basePositionPct * baseMultiplier * qualityMultiplier * regimeMultiplier, 4);
  ...
  if (portfolioBudget.asset_class_concentration_pct >= portfolioBudget.asset_class_concentration_cap_pct) {
  ...
  return {
    ...
    decision,
    base_position_pct: round(basePositionPct, 4),
    recommended_position_pct: decision === 'reduce' ? round(recommendedPositionPct * 0.85, 4) : recommendedPositionPct,
    reasons,
    risk_explanation: {
      regime_posture: regimeState?.state?.recommended_user_posture || '--',
      quality_bucket: tradeBucket,
...
export function buildRiskBucketSystem({
  ...
  const totalExposure = round(sum(tradableSignals.map((item) => Number(item.position_advice?.position_pct ?? item.position_size_pct ?? 0))), 4);
  const totalActiveRisk = round(
    sum(
      tradableSignals.map((item) => {
        const positionPct = Number(item.position_advice?.position_pct ?? item.position_size_pct ?? 0);
  ...
  const dailyPnlPct = round(
    (trades || [])
      .slice(0, 12)
      .reduce((acc, item) => acc + Number(item.pnl_pct || 0), 0),
    4
  );

  const correlation = correlationFootprint(tradableSignals);
  const marketConcentration = concentrationBy(tradableSignals, (item) => String(item.market || 'unknown'));
  const assetClassConcentration = concentrationBy(tradableSignals, (item) => String(item.asset_class || 'unknown'));
  const drawdownProxy = round(Math.abs(Number(championState?.safety?.cards?.portfolio?.score ?? 80) - 100) * 0.24, 4);
  ...
  const portfolioBudget = {
    ...
    remaining_total_exposure_pct: round(Math.max(0, userBucket.total_exposure_cap_pct - totalExposure), 4),
    max_total_active_risk_pct: userBucket.max_total_active_risk_pct,
    used_total_active_risk_pct: totalActiveRisk,
    remaining_total_active_risk_pct: round(Math.max(0, userBucket.max_total_active_risk_pct - totalActiveRisk), 4),
  ...
  const openSlots = Math.max(
    0,
    userBucket.max_concurrent_trades - openPositions
  );
  ...
  return {
    ...
    explainability: {
      why_allowed: 'Trade is allowed when setup quality is A/B and portfolio budgets remain within limits.',
      why_reduced: 'Size is reduced when regime posture is REDUCE or setup quality is non-A.',
      why_blocked: 'Trade is blocked when quality fails, posture is SKIP, or risk budgets are exhausted.'
    }
  };
}
