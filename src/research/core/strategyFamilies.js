...
const TEMPLATE_DEFAULTS = Object.freeze({
  ...
  compatible_filters: ['liquidity_filter', 'regime_filter', 'risk_bucket_filter', 'conflict_filter'],
    ...
    templates: [
      {
        ...
        activation_conditions: ['trend_score >= 0.58', 'breakout confirmed by volume', 'not risk_off'],
  ...
  },
  {
    ...
    templates: [
      ...
      },
      {
        ...
        expected_holding_horizon: '1-4 bars',
        cost_sensitivity_assumptions: 'Medium; risk reduction logic has priority over edge capture.',
    ...
    templates: [
      {
        ...
        required_inputs: ['funding_rate', 'funding_zscore', 'basis_annualized', 'open_interest_change'],
        tunable_parameters: [
      ...
      },
      {
        ...
        activation_conditions: ['exchange divergence exceeds stress threshold with executable depth'],
        lifecycle_stage: LIFECYCLE_STAGE.DRAFT
      },
      {
        strategy_template_name: 'liquidity_stress',
        supported_asset_classes: ['CRYPTO'],
        required_inputs: ['depth_imbalance', 'spread_bps', 'liquidation_pressure', 'velocity_shock'],
        tunable_parameters: [
    ...
    templates: [
      {
        ...
        tunable_parameters: [
          { name: 'event_risk_cap', default: 0.7, min: 0.3, max: 0.99 }
        ],
      ...
      },
      {
        ...
        tunable_parameters: [
          { name: 'flow_signal_min', default: 0.6, min: 0.3, max: 0.95 }
        ],
        compatible_regimes: ['trend', 'range', 'high_volatility'],
        expected_holding_horizon: 'overlay',
        cost_sensitivity_assumptions: 'N/A overlay',
      ...
      },
      {
        ...
        tunable_parameters: [
          { name: 'sentiment_weight_cap', default: 0.25, min: 0.05, max: 0.5 }
        ],
        compatible_regimes: ['all'],
        expected_holding_horizon: 'overlay',
        cost_sensitivity_assumptions: 'N/A overlay',
      ...
      },
      {
        ...
        tunable_parameters: [
          { name: 'vol_surface_alert_min', default: 0.65, min: 0.2, max: 0.99 }
        ],
        compatible_regimes: ['high_volatility', 'risk_off'],
        expected_holding_horizon: 'overlay',
        cost_sensitivity_assumptions: 'N/A overlay',
...
function lifecycleFromLegacyStage(stage) {
  ...
  if (normalized === 'candidate' || normalized === 'paper' || normalized === 'canary') return LIFECYCLE_STAGE.CANARY;
  if (normalized === 'testing' || normalized === 'challenger' || normalized === 'shadow') return LIFECYCLE_STAGE.SHADOW;
...
function flattenTemplates(families) {
  ...
  for (const family of families) {
    for (const template of family.templates) {
      const defaults = {
        validation_requirements: template.validation_requirements || TEMPLATE_DEFAULTS.validation_requirements,
...
function mapExistingStrategies(strategyRegistry = [], asOf) {
  return (strategyRegistry || []).map((row) => ({
    ...
    strategy_version: row?.notes?.find?.((item) => String(item).startsWith('version='))?.split('=')[1] || 'v1',
...
export function buildStrategyFamilyRegistry({
  ...
  const familyCoverage = FAMILY_REGISTRY.map((family) => ({
    ...
    supported_asset_classes: [...new Set(family.templates.flatMap((item) => item.supported_asset_classes))]
  }));
  ...
  return {
    ...
    extensibility_note: 'Add a new family by appending a family object with template metadata and activation conditions.'
  };
}
