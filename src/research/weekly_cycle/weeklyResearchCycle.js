...
function topReasons(rows = [], field = 'reason', count = 3) {
  return (rows || []).slice(0, count).map((row) => row[field]).filter(Boolean);
}
...
export function buildWeeklyResearchCycle({
  ...
  const promotions = (strategyGovernance?.decisions || []).filter((row) => row.action === 'PROMOTE');
  const demotions = (strategyGovernance?.decisions || []).filter((row) => ['DEMOTE', 'ROLLBACK', 'RETIRE'].includes(row.action));
  ...
  const report = {
    ...
    strategy_promotions_demotions: {
      promotions: promotions.map((row) => ({ strategy_id: row.strategy_id, to_stage: row.to_stage })),
    ...
    portfolio_insights: {
      ...
      diversification_score: portfolioSimulation?.diagnostics?.diversification_contribution?.diversification_score || 0
    },
...
export function buildWeeklyResearchReportMarkdown(report = {}) {
  ...
  lines.push(summaryLine('Generated candidates', report.discovery_results?.generated_candidates ?? 0));
  lines.push(summaryLine('Survivors after validation', report.discovery_results?.survivors_after_validation ?? 0));
  lines.push(summaryLine('Promoted to SHADOW', report.discovery_results?.promoted_to_shadow ?? 0));
  lines.push(summaryLine('Discovery success rate', report.discovery_results?.discovery_success_rate ?? 0));
  lines.push('');

  lines.push('## Validation Results');
  lines.push(summaryLine('Evaluated strategies', report.validation_results?.evaluated_strategies ?? 0));
  ...
  lines.push(summaryLine('Fragile strategies', (report.validation_results?.fragile_strategies || []).join(', ') || 'none'));
  lines.push('');

  lines.push('## Promotions / Demotions');
  lines.push(summaryLine('Promotion count', report.strategy_promotions_demotions?.promotions?.length ?? 0));
  lines.push(summaryLine('Demotion/retire count', report.strategy_promotions_demotions?.demotions?.length ?? 0));
  lines.push(summaryLine('Degradation alerts', report.strategy_promotions_demotions?.degradation_alert_count ?? 0));
  lines.push('');
  ...
  lines.push(summaryLine('Bottleneck', report.signal_density_issues?.bottleneck_stage ?? 'unknown'));
  lines.push(summaryLine('Top no-trade reasons', (report.signal_density_issues?.no_trade_top_reasons || []).join(', ') || 'none'));
  lines.push('');
  ...
  lines.push(summaryLine('Warnings', (report.regime_insights?.warnings || []).join(' | ') || 'none'));
  lines.push('');

  lines.push('## Shadow Insights');
  lines.push(summaryLine('Shadow records', report.shadow_insights?.total_shadow_records ?? 0));
  lines.push(summaryLine('Strictness watch count', report.shadow_insights?.strictness_watch_count ?? 0));
  lines.push(summaryLine('Missed opportunity ratio', report.shadow_insights?.missed_opportunity_ratio ?? 0));
  lines.push(summaryLine('Top filter reasons', (report.shadow_insights?.top_filter_reasons || []).join(', ') || 'none'));
  lines.push('');
  ...
  lines.push(summaryLine('Diversification score', report.portfolio_insights?.diversification_score ?? 0));
  lines.push('');
