...
function funnelInsights(funnel = {}, insights = []) {
  ...
  if (ratio <= 0.25) {
    pushInsight(insights, {
      ...
      message:
        'Signal funnel executable ratio is low; review score/risk/conflict filters and bounded threshold relaxations.',
  ...
  if (bottleneck === 'risk_filtered') {
    pushInsight(insights, {
      ...
      message:
        'Risk filter currently removes the largest share of candidates; evaluate bucket limits and concentration constraints.',
      evidence_refs: [
        'signal_funnel_diagnostics.bottleneck',
        'risk_bucket_system.decision_summary',
      ],
    });
  }
  ...
  if (Number(funnel?.threshold_sensitivity?.near_threshold_share || 0) >= 0.35) {
    pushInsight(insights, {
      ...
      message:
        'Many candidates sit near score thresholds; adaptive threshold policy may improve density without regime-blind loosening.',
...
function shadowInsights(shadow = {}, insights = []) {
  if (Number(shadow?.missed_opportunity_ratio || 0) >= 0.25) {
    pushInsight(insights, {
      ...
      message:
        'Shadow logs suggest rejected opportunities may contain usable signal; test reduced-size or bounded threshold variants.',
      evidence_refs: [
        'shadow_opportunity_log.missed_opportunity_ratio',
        'shadow_opportunity_log.under_traded_family_regime_matrix',
      ],
    });
  }
}
...
function validationInsights(walkForward = {}, discovery = {}, insights = []) {
  ...
  if ((coverageGaps.regime_gaps || []).length) {
    pushInsight(insights, {
      ...
      message:
        'Discovery diagnostics show regimes with generated candidates but no promotions; add targeted hypotheses/templates.',
  ...
  const noisyFamily = recurringFails.find(
    (row) => Number(row.failed_count || 0) >= 5 && Number(row.promoted_count || 0) <= 1,
  );
  if (noisyFamily) {
    pushInsight(insights, {
      severity: 'medium',
...
function degradationInsights(governance = {}, regimeState = {}, insights = []) {
  const degraded = (governance?.strategies || []).filter(
  ...
  if (String(regimeState?.state?.primary || '').toLowerCase() === 'high_volatility') {
    pushInsight(insights, {
      ...
      message:
        'Current regime is high volatility; momentum starves while transition/defensive families should be emphasized.',
...
function portfolioInsights(portfolio = {}, insights = []) {
  const divScore = Number(
    portfolio?.diagnostics?.diversification_contribution?.diversification_score || 0,
  );
  ...
  if (divScore < 0.45) {
    pushInsight(insights, {
      ...
      message:
        'Diversification score is below target; promote uncorrelated families and trim concentrated allocation clusters.',
  ...
  if (drawdown > 0.22) {
    pushInsight(insights, {
      ...
      message:
        'Simulated drawdown remains elevated; tighten risk budgets and reduce allocation to high-turnover/fragile strategies.',
      evidence_refs: [
        'portfolio_simulation.metrics.drawdown',
        'portfolio_simulation.diagnostics.marginal_strategy_impact',
      ],
    });
  }
}
...
function hypothesisSuggestions(discovery = {}, insights = []) {
  ...
  const weak = byHypothesis
    .filter((row) => Number(row.generated || 0) >= 4 && Number(row.promoted_to_shadow || 0) === 0)
    .slice(0, 3);
...
export function buildAiResearchCopilot({
  ...
  return {
    ...
    regime_coverage_warnings: prioritized.filter(
      (item) =>
        item.title.toLowerCase().includes('regime') ||
        item.message.toLowerCase().includes('regime'),
    ),
    validation_warnings: prioritized.filter(
      (item) =>
        item.title.toLowerCase().includes('fragile') ||
        item.message.toLowerCase().includes('walk-forward'),
    ),
