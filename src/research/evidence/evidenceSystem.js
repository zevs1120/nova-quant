...
function mergePromotionHistory({
  candidateId,
  strategyId,
  discoveryDecisions = [],
  governanceDecisions = [],
...
function buildCandidateEvidence({
  ...
  const matchedOpportunity = (productOpportunities || []).find(
    (item) =>
      String(item.strategy_template || '').toLowerCase() ===
        String(candidate.template_id || '').toLowerCase() ||
  ...
  const stageMetrics = Object.fromEntries(
    validationStages.map((row) => [row.stage, row.metrics || {}]),
  );
  ...
  return {
    ...
    regime_performance: {
      ...
      validation_regime_segmentation:
        stageMetrics.stage_3_robustness_tests?.regime_segmentation || [],
    ...
    assumption_profile:
      executionRealism?.assumption_profile ||
      walkForward?.replay_context?.assumption_profile ||
      null,
    cost_realism_notes: [
...
function buildGovernedStrategyEvidence({
  ...
  return {
    ...
    validation_summary: {
      status:
        walkForward?.verdict?.promotion_readiness === 'pass' ? 'pass_to_scoring' : 'legacy_or_hold',
  ...
  return {
    ...
    summary: {
      ...
      average_evidence_quality_score: allEvidence.length
        ? round(
            allEvidence.reduce((acc, row) => acc + row.evidence_quality_score, 0) /
              allEvidence.length,
            4,
          )
        : 0,
